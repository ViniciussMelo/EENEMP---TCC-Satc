...
if (isset($_POST['cadastrar']))
{
  ...
  $avUsuario = $_POST ['avUsu'];
  $nota 	 = $_POST ['notaU'];
  $usuario 	 = 'Professor';

	   $verifica = ("SELECT emailUsuario FROM avaliacaousuario where emailUsuario = '$email'");

       $res = mysql_query($verifica);

       if (mysql_num_rows ( $res ) == 0) 
      {         
				 $sql = "INSERT INTO avaliacaousuario (usuario,avaliacao, nota, emailUsuario) VALUES ('$usuario','$avUsuario','$nota','$email')";
				 $res = mysql_query($sql);
          ...
          if ($res)
		{
			  ...
			  window.location.href='verAvaliacoesProf.php';</script>";
			  die();
		  }
		  else
		  {
		...
		echo"<script language='javascript' type='text/javascript'>
		alert('Você já cadastrou sua avaliação de usuário e somos gratos por isso!!');
		window.location.href='verAvaliacoesProf.php';</script>";
		die();		
	  }
}
...
<html>
<head>
<title>EENEMP/Avaliação de usuario Professor</title>
<meta charset="utf-8">
...
<h3 class="tituloaluno">Tire suas duvidas sobre o ENEM!</h3>
	...
	<div class="cadastro">
     ...
     <form name="formulario" onSubmit="return validaCampo();" class="formulario" method="POST" action="avDeUsuarioProf.php" >
        <fieldset>
            <br>
			<input type="text" readOnly="true" id="emailU" name="emailU" placeholder="<?php echo $_SESSION["logadoProf"]?> " >
